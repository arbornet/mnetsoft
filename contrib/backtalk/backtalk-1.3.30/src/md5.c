/* This code is not covered in any way by the Backtalk copyright or license.
 * Relevant terms are below.  Actually, most modern Unix systems have md5
 * routines somewhere in their system libraries, but there is enough variation
 * in the interfaces, locations of header files and library names that it is
 * easier to just include a copy in the Backtalk distribution than to adapt to
 * all of them.
 */

#include "common.h"

#if defined(NOISE_FILE)
 ...
 */
void
MD5Update (register MD5_CTX * mdContext, unsigned char *inBuf,
	   unsigned int inLen)
{
	register int i, ii;
	int mdi;
 ...
 */
void
Transform (register UINT4 * buf, register UINT4 * in)
{
