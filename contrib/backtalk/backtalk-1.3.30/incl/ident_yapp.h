...
/* IDENTITY MODULE INTERFACE - Yapp Compatible Version
 *
...
/* CONSTANT:  IDENT_STOREEMAIL
 *
 *     If defined, our ident database includes the user's email address.
 *     Otherwise, email addresses are kept in .backtalk files.  If there is
 *     an email address, it will be stored in the pw_shell field will contain
 *     the email address instead of whatever would normally be there.
 */

#define IDENT_STOREEMAIL
...
#undef HAVEIDENT_WALKPW		/* Use auth_walk() */
...
#undef HAVEIDENT_WALK		/* Use auth_walk() */
...
/* FUNCTION: ident_newemail(pwd, email)
 *
 *     Change the user's email address.  pwd will have the full set of old
 *     ident information including login name and full name.
 *
 *     Never used unless IDENT_STOREEMAIL is defined.  Optional even then,
 *     but if not provided, email addresses cannot be editted through
 *     Backtalk.
 */

#define HAVEIDENT_NEWEMAIL
 ...
 *           ident_add(login,uid,gid,fname,dir,email)
 *
 *     Add a new entry into the identity database.  Syntax varies depending
 *     on whether IDENT_STOREEMAIL is defined.
 *
