...
static void dump(time_t m);
static void colonize(char *line, int slot);
static void wildrexit(char *string);
static void wildexit(char *string);
static void tameexit(void);
static void fudge(char *buf);
static time_t mdy2t(int m, int d, int y);
static time_t midnight(time_t t);
static void newday(time_t m, time_t t, char headchar);
...
static struct entry *
lookup(struct utmp * up, char doenter)
{
	struct entry *ep;
	char *cp;
	int i;
	...
	if (i == 0)
		...
		ep = (struct entry *) realloc((char *) entries,
	...
	(void) snprintf(ep->l, sizeof(ep->l), "%*s", dayslots, "");
	ep->u.ut_time = 0;
	ep->u.ut_name[0] = '\0';
	ep->marked = FALSE;
	ep->daysecs = 0;
...
static char *wname = "/var/log/wtmp";
...
static int
qtime(const void *up1, const void *up2)
{
	long diff;
	...
	*/
	for (unext = ubase = utmp; unext < ubeyond; ++unext) {
		...
		if (unext->ut_time == 0 || unext->ut_line[0] == '\0')
			continue;
		/* Throw out all the ftpNNNN and uuNNNN entries. Gregc */
		else if ((strncmp(unext->ut_line, "ft", 2) == 0)
			 || (strncmp(unext->ut_line, "uu", 2) == 0)) {
			/* printf("Line: %s\n", unext->ut_line); */
			continue;
		}
		/* Throw out all the accounting entries. Gregc */
		else if (strncmp(unext->ut_line, "acct", 4) == 0)
			continue;
		else if (ubase == unext)
			++ubase;
		else
	...
	for (; unext < ubeyond; ++unext, lastm = m) {
		...
		if (OLDTLINE(unext->ut_line)) {
			...
			if (unext->ut_line[0] == NEWTCHAR ||
				...
				newday(m, unext->ut_time, NEWTCHAR);
			continue;
		}
		if ((ep = lookup(unext, !didttys)) == NULL)
			continue;
...
static int
qline(const void *ep1, const void *ep2)
{
...
dump(time_t m)
{
	...
	for (ep = entries; ep < ebeyond; ++ep) {
		...
		(void) snprintf(buf, sizeof(buf), "%2.2s%4.1f",
				ttyabbr(ep->u.ut_line), d);
		if (buf[2] != ' ')
			(void) snprintf(buf, sizeof(buf), "%2.2s%4.0f",
					ttyabbr(ep->u.ut_line), d);
...
static void
newday(time_t m, time_t t, char headchar)
{
	struct entry *ep;
	int i, j, slot;
	time_t t2;
	...
	for (i = 0; i < dayhours; ++i) {
		...
		if (j == 12 && lflag)
			if (i == 0)
				(void) strlcpy(buf, "Mid.-", sizeof(buf));
			else
				(void) strlcpy(buf, "Noon-", sizeof(buf));
		else
			(void) snprintf(buf, sizeof(buf), "%d-----", j);
		buf[hourslots] = '\0';
		(void) strlcat(headline, buf, sizeof(headline));
	}
	if (t == 0)
		slot = 0;
	else {
